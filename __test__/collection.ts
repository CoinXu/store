...
describe("storeCollectionCreator middleware", function() {
  ...
  const scheduler: CollectionScheduler<User, { message: string }> = function(
    action: Action,
    collection,
    next
  ) {
  ...
  const store = new Store<{
    user: CollectionState<User, { message: string }>;
  }>();
  storeCollectionCreator(Desc, store);

  it("add one item to collection", function(done) {
    store.dispatch({ type: "ADD", payload: { name: "a", id: 0 } }, function(
      state
    ) {
